...
Player::Player(Vector2 size, Vector2 pos, Vector2 velocity, Rectangle playerPointColl, float mass) {
    this->size = size;
    this->pos = pos;
    this->velocity = velocity;
    this->playerPointColl = playerPointColl;
    this->mass = mass;
}

void playerDrawOutScreen(Vector2 pos, Vector2 player, Color color) {
    if (pos.x < player.x) {
        DrawRectangle((int) pos.x + Globals::WIDTH, (int) pos.y, (int) player.x, (int) player.y, color);
    }
    if (pos.x + player.x > Globals::WIDTH) {
        DrawRectangle((int) pos.x - Globals::WIDTH, (int) pos.y, (int) player.x, (int) player.y, color);
    }
}

void playerGetVelocity(Vector2 *position, Vector2 *velocity, float mass, float timestep) {
    Vector2 force = {-velocity->x * 4, -Globals::gravity * mass};
    ...
    Vector2 newForce = {0, -Globals::gravity * mass};
